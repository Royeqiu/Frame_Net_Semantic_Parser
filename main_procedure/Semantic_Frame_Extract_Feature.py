...
from Frame_net import FrameNet_Loader
...
sfmf.nlp = nlp
sentences = dbo.get_semantic_frame_model_sentence(True)
for new_sentence in dbo.get_semantic_frame_model_sentence():
...
for i, sentence in enumerate(sentences):
    if i % 50 == 0:
        print('%s sentences have been processed' % (i))
    ...
    dbo.add_frame_model_feature(feature_lists, lexicon_text)
    dbo.add_feature_dictionary([str(semantic_frame_id)])
    dbo.add_frame_model_label(semantic_frame_id, lexicon_text)
    vector_list = []
